...
public class SLinkedList<E> {
	...
	public void add(int index, int data, int tiempo){  
		...
		if(index == 0){
			...
			Node<E> nuevo = new Node(ant.getNext(),data, tiempo);
			ant.setNext(nuevo);
			size++;
		}
	}
	...
	public void addFirst(int data){  
			this.firstNode = new Node(this.firstNode, data, 0);
			this.size++; 
	}
	...
	public float getVelS(int index){
		if(this.size == 0){
			throw new NoSuchElementException ("Empty List!");
		}else if(index == 0){
			return firstNode.getVelS();
		}else{
			Node<E> ant = null;
			ant = firstNode;
			for(int i=0;i<index;i++){
